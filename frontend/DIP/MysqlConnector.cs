...
namespace DIP
{
    public class MysqlConnector
    {
        ...
        public void Testing_Insert(int key)
        {
            cmd.CommandText = string.Format("Update Semaphore set test = 0", key);
            cmd.ExecuteNonQuery();
        }

        public DataTable Testing()  //for testing
        {
            cmd.CommandText = "select * from Semaphore";
            adp = new MySqlDataAdapter(cmd);
            DataTable tab = new DataTable();
            adp.Fill(tab);
            return tab;
